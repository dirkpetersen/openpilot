...
def act(*a, **k):
  """netcosttier2pnw: decide() now returns (action, ssid) so the daemon cannot bring up a different
  network than the one the cost ladder ranked. These tests predate the ssid and assert on the action
  only, which is still exactly what they are about."""
  return decide(*a, **k)[0]
...
class TestDecideTetheringOnPriority:
  def test_priority_in_range_and_saved_switches_to_wifi(self):
    ...
    assert act(True, PRIORITY_SSID, [PRIORITY_SSID, "other"], [PRIORITY_ID], HOTSPOT_CONNECTION_ID) == "up_priority"

  def test_priority_already_active_noop(self):
    # already on the priority network -> idempotent, do nothing
    assert act(True, PRIORITY_SSID, [PRIORITY_SSID], [PRIORITY_ID], PRIORITY_ID) == "noop"

  def test_priority_absent_brings_hotspot_up(self):
    # tethering on, priority configured but not in range -> hotspot should be up
    assert act(True, PRIORITY_SSID, ["other"], [PRIORITY_ID], None) == "up_hotspot"

  def test_priority_absent_hotspot_already_up_noop(self):
    assert act(True, PRIORITY_SSID, ["other"], [PRIORITY_ID], HOTSPOT_CONNECTION_ID) == "noop"
  ...
  def test_priority_in_range_but_not_saved_brings_hotspot_up(self):
    ...
    assert act(True, PRIORITY_SSID, [PRIORITY_SSID], [], None) == "up_hotspot"

  def test_priority_not_saved_hotspot_already_up_noop(self):
    assert act(True, PRIORITY_SSID, [PRIORITY_SSID], [], HOTSPOT_CONNECTION_ID) == "noop"
...
class TestDecideEdgeCases:
  def test_priority_only_named_ssid_interrupts_hotspot(self):
    ...
    assert act(True, PRIORITY_SSID, ["cafe"], [other_id], HOTSPOT_CONNECTION_ID) == "noop"
  ...
  def test_switch_from_wrong_client_to_priority(self):
    ...
    other_id = priority_connection_id("cafe")
    assert act(True, PRIORITY_SSID, [PRIORITY_SSID, "cafe"], [PRIORITY_ID, other_id], other_id) == "up_priority"
