...
| 3 | arm lifetime | **20 s** | `lateralOnly` can persist indefinitely; a pending resume must not. Bounds "a resume arriving minutes after the brake". |
...
| `phase` | Meaning |
|---|---|
| `arm` | A brake press left MADS steering alone; the window is open. `setMs` null ⇒ gate 6 already failed. |
